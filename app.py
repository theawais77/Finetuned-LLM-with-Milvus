...
def stats_endpoint():
    ...
    try:
        stats = {
            "timestamp": datetime.utcnow().isoformat(),
            "initialization_status": trading_assistant._initialized,
            "scheduler_running": trading_assistant.scheduler is not None and trading_assistant.scheduler.running,
        }
        
        # Add Milvus collection stats if available
        if trading_assistant._initialized and trading_assistant.collection:
            try:
                stats["documents_count"] = trading_assistant.collection.num_entities
            except Exception as e:
                stats["documents_count"] = f"error: {str(e)}"
        
        # Add circuit breaker stats
        stats["circuit_breakers"] = {
            "binance": {
                "state": trading_assistant.binance_service.circuit_breaker.state,
                "failure_count": trading_assistant.binance_service.circuit_breaker.failure_count
            },
            "llm": {
                "state": trading_assistant.llm_service.circuit_breaker.state,
                "failure_count": trading_assistant.llm_service.circuit_breaker.failure_count
            }
        }
...
def reset_endpoint():
    """Reset services (for debugging)"""
    try:
        # Reset circuit breakers
        trading_assistant.binance_service.circuit_breaker.failure_count = 0
        trading_assistant.binance_service.circuit_breaker.state = 'CLOSED'
        trading_assistant.llm_service.circuit_breaker.failure_count = 0
        trading_assistant.llm_service.circuit_breaker.state = 'CLOSED'
        
        # Reset connection flags
        trading_assistant.milvus_service._connection_established = False
        trading_assistant.llm_service._model_warmed = False
        
        logger.info("Services reset successfully")
