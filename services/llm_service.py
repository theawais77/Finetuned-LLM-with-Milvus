...
class LLMService:
    ...
    def _make_request(
        self, prompt: str, max_tokens: int = 256, timeout: int = 180
    ) -> Optional[Dict]:
    ...
    def generate_response(
        self, question: str, context: str, market_data: Optional[Dict] = None
    ) -> Dict:
        ...
        try:
            ...
            return {"answer": fallback, "status": "fallback", "error": str(e)}

    def _build_prompt(
        self, question: str, context: str, market_data: Optional[Dict]
    ) -> str:
...
Instructions:
- Answer in detail
- Make comparison of data if needed
...
"""
    ...
    def _format_market_data(self, market_data: Dict) -> str:
        ...
        for symbol, data in market_data.items():
            if isinstance(data, dict) and "price" in data:
                formatted.append(
                    f"{symbol}: ${data['price']} ({data.get('timestamp', 'N/A')})"
                )
    ...
    def _generate_fallback(
        ...
        if context and len(context.strip()) > 20:
            return (
                "I'm having trouble processing your request fully, but based on available data: "
                + context[:200]
                + "..."
            )
